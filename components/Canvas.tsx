...
const Canvas: React.FC<CanvasProps> = ({ displayImageUrl, onStartOver, isLoading, loadingMessage, onSelectPose, poseInstructions, currentPoseIndex, availablePoseKeys }) => {
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4 relative animate-zoom-in group">
      ...
      <button 
          onClick={onStartOver}
          className="absolute top-4 left-4 z-30 flex items-center justify-center text-center bg-white/40 backdrop-blur-md border border-y2k-blue/30 text-y2k-blue font-bold py-2 px-4 rounded-full transition-all duration-200 ease-in-out hover:bg-white/80 hover:border-y2k-blue/50 active:scale-95 text-sm"
      >
      ...
      <div className="relative w-full h-full flex items-center justify-center">
        {displayImageUrl ? (
          <img
            ...
            className="max-w-full max-h-full object-contain transition-opacity duration-500 animate-fade-in rounded-lg shadow-2xl shadow-y2k-blue/10"
          />
        ) : (
            <div className="w-[400px] h-[600px] bg-white/30 border border-y2k-blue/20 rounded-lg flex flex-col items-center justify-center">
              <Spinner />
        ...
        <AnimatePresence>
          {isLoading && (
              <motion.div
                  className="absolute inset-0 bg-y2k-bg/80 backdrop-blur-md flex flex-col items-center justify-center z-20 rounded-lg"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
              >
      ...
      {displayImageUrl && !isLoading && (
        ...
        >
          ...
          <AnimatePresence>
              {isPoseMenuOpen && (
                  <motion.div
                      ...
                      className="absolute bottom-full mb-3 w-64 bg-white/60 backdrop-blur-xl rounded-xl p-2 border border-y2k-blue/30"
                  >
                      <div className="grid grid-cols-2 gap-2">
                          {poseInstructions.map((pose, index) => (
                              <button
                                  ...
                                  className="w-full text-left text-sm font-medium text-gray-800 p-2 rounded-md hover:bg-y2k-blue/20 disabled:opacity-50 disabled:bg-y2k-blue/20 disabled:font-bold disabled:cursor-not-allowed"
                              >
                                  {pose}
                              </button>
                          ))}
          ...
          <div className="flex items-center justify-center gap-2 bg-white/40 backdrop-blur-xl rounded-full p-2 border border-y2k-blue/30">
            <button 
              ...
              disabled={isLoading}
            >
              <ChevronLeftIcon className="w-5 h-5 text-y2k-blue" />
            </button>
            <span className="text-sm font-bold text-y2k-blue w-48 text-center truncate" title={poseInstructions[currentPoseIndex]}>
            ...
            <button 
              ...
              disabled={isLoading}
            >
              <ChevronRightIcon className="w-5 h-5 text-y2k-blue" />
            </button>
          </div>
        </div>
      )}
